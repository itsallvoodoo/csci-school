...
class Board:
    def __init__(self,w,h,player1,player2,game_id):
        ...
        self.refresh_button = Button("Refresh",Point(152,80),Point(180,90))
        self.refresh_button.draw(self.win)

        self.game_id_text = Text(Point(175,175),game_id)
        self.game_id_text.draw(self.win)
        
        # TODO
        # Need to add code here to display and keep track of the game id
        # The game id is now a new parameter (see top of this function)
        # At the end of this code, this object should have an integer attribute
        # called game_id and a Text object attribute called game_id.
        # My code was three lines for comparison
        ...
        self.player2_score = Text(Point(-15,160),str(player2.getScore()))
        self.player2_score.draw(self.win)
        
        self.player1_score = Text(Point(160,-15),str(player1.getScore()))
    ...
    def __getstate__(self):
        # We had to override the getstate function, which gets called by pickle
        # Basically, we needed to delete all references to graphical objects as they
        # cannot be pickled
        result = self.__dict__.copy()
        result['win'] = None
        result['quit_button'] = None
        result['submit_button'] = None
        result['check_button'] = None
        result['pass_button'] = None
        result['refresh_button'] = None
        result['message'] = None
        result['player2_score'] = None
        result['player1_score'] = None
        result['player2_name_text'] = None
        result['player1_name_text'] = None
        result['game_id_text'] = None
        return result
    ...
    def update_screen(self):
        ...
        self.game_id_text.setText(str(self.game_id))

    def undraw(self):
        for i in range(len(self.tile_list)):
            self.tile_list
        # TODO: Write an undraw function that undraws all of the tiles and markers
        # This includes all of tiles that have not yet been handed out and those
        # tiles that are in the players hands
    ...
    def take_turn(self,player):        
        ...
        while True:
            ...
            # TODO: Write code that makes sure it is this player's trun
            # If it is not this player's turn and they clicked on refresh button -> return 'refresh'
            # If it is not this player's turn and they clicked on quit -> return 'quit'
            # else continue to the next iteration of this loop
                ...
                for i in range(len(self.markers)):
                    for j in range(len(self.markers[i])):
                        if self.markers[i][j].clicked(p):
                            ...
                            mark_clicked = True
                            if self.markers[i][j].getTile() != None:
                                self.markers[i][j].getTile().setMarker(None)
                            self.markers[i][j].setTile(tile_selected)
                            if tile_selected.getMarker() != None:
                                tile_selected.getMarker().setTile(None)
            ...
            if tile_clicked or mark_clicked: # Done
                ...
                self.check()                    
            elif self.refresh_button.clicked(p):
                return "refresh"
